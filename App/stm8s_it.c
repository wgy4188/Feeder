...
static uint8_t Increase_Flag=0,Toggle_V=0;
...
INTERRUPT_HANDLER(TIM2_UPD_OVF_BRK_IRQHandler, 13)
{
      ...
      */
      ...
      if(TIM2->SR1&0x01)
	  {
        TIM2->SR1 &=~(uint8_t)(0x01);//清除中断标志

        //GPIO
        if(Increase_Flag == 0 ){
          if(Tick_clock < 20) {Tick_clock++;}
          else {Increase_Flag = 1;Toggle_V=0;}
        }
        else if(Increase_Flag == 1){
          if(Tick_clock > 0)  {Tick_clock--;}
          else {Increase_Flag = 0;Toggle_V=1;}
        }

        if(Tick_clock  == 6){
          if(Toggle_V == 0){
            //  GPIOD->ODR |=  0x01;  Toggle_V = 1;
          }else if(Toggle_V == 1){
            //  GPIOD->ODR &= ~0x01;  Toggle_V = 0;
          }
        }

        if(Display_delay>0){Display_delay--;}
        if(Menu_Setting.System_Reset_Delay > 0){Menu_Setting.System_Reset_Delay--;}
		if(Menu_Setting.System_Set_Delay>0){Menu_Setting.System_Set_Delay--;}

		if(Roll_Motor_Delay > 0){Roll_Motor_Delay--;}else{Roll_Motor_Delay=0;}
		if(Vibra_Motor_Delay > 0){Vibra_Motor_Delay--;}else{Vibra_Motor_Delay=0;}
